...
from app.models import db, Set  # импортируем из папки app
    ...
    with app_db.app_context():
        try:
            ...
            if new_value is not None:
                print(f"исходные значения настроек: {new_value.id} {new_value.start_string}"
                ...
                new_value.search_page_number = start_settings[2]    # количество запрашиваемых страниц поиска
            else:
                print("строка id==1 отсутствует в БД")
                new_value = Set(start_string=start_settings[0],
            ...
            new_value = db.session.query(Set).filter(Set.id == 1).first()
            print(f"новые значения настроек:    {new_value.id} {new_value.start_string}"
                  f" {new_value.wait_interval} {new_value.search_page_number}")
